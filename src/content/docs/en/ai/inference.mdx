...
A published model can also carry a **System prompt** for the Codex harness. **Append to harness default** keeps Codex's default instructions, including its tool and patch-format rules, and adds the prompt after them; **Replace harness default** substitutes them entirely, so a replacement must restate any rules the model still needs. The prompt reaches a device on its next model-catalog refresh and Codex restart.
...
Gateway computes the multiplier per model across that model's subscription accounts. A quota window that a provider limits to one model family counts only for that family. Missing quota data leaves the multiplier at `1×`; stale, empty, or exhausted discovered quota can force the protective `8×` value.
...
Gateway reserves capacity before sending a request, and the full configured subscription limit is spendable. A request admitted while the user still has a positive balance always runs to completion, so the final turn can settle slightly past the limit; Gateway never shortens an admitted request. The next request after the balance is exhausted is denied with the window's recovery time.
