...
An agent whose user may work only in some folders, Nodes, or resources stays inside them. When the connection is limited, Gateway adds a summary of that access to the MCP server instructions at connect time. The `using-gateway` and `access-control` skills tell the agent to call `get_my_access`, or read `gateway://access`, when a list comes back empty or a create at the root is refused, and to pass `folderId` when creating; a refused create also names the folders it may use. See [Find out what the caller can access](/en/integrations/api-and-mcp/#find-out-what-the-caller-can-access).
