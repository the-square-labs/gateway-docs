...
AI Workspace chat is available on every plan. Plan Mode, guided Scenarios, and AI Sandboxes require Personal or higher; on Community, a Plan Mode request is refused with `LICENSE_ENTITLEMENT_REQUIRED` and the Scenario list is empty. See [Plans and entitlements](/en/reference/plans-and-entitlements/#feature-matrix).
