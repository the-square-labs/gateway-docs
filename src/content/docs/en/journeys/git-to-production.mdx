...
Before starting, prepare the [source-control integration](/en/integrations/source-control/) and [Build Worker](/en/docker/git-builds/). For a static site, also review [Pages Git deployments](/en/pages/git-deployments/).
...
| --- | --- | --- |
| Repository is not discovered | Connector authorization, repository allowlist, branch, application root | [Source-control integrations](/en/integrations/source-control/) |
| Build Worker does not accept the job | Node status, capabilities, plan, registry write access | [Git sources and Build Workers](/en/docker/git-builds/) |
| Build is rejected | Policy findings and blocking threshold in the build record | [Git sources and Build Workers](/en/docker/git-builds/) |
| Artifact exists but deployment fails | Deployment Task, target Node, logs, health check | [Tasks, events, and audit](/en/reference/tasks-and-audit/) |
| Image cannot be pulled | Registry reachability, digest, and stored credentials | [Registries and image trust](/en/docker/registries/) |
