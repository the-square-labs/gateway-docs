...
If SIEM is disabled, destination configuration and historical terminal records remain. After a license grace period ends, SIEM forwarding pauses: configuration, delivery history, and records already queued are kept, but audit events that occur while forwarding is paused are not queued; forwarding resumes on renewal without reconfiguration. Structured-log ingestion keeps working after grace. See [Downgrade and expiry](/en/reference/plans-and-entitlements/#downgrade-and-expiry). Confirm the receiver's last accepted event and Gateway's queue age before resuming normal operation.
