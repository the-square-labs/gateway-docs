...
Any action scope implies view of the same resources: `proxy:edit:<route-id>` shows that Route, and `docker:containers:manage:<node-id>` shows the containers on that Node. Creation scopes are the exception: they imply no view at all, whether they are broad or restricted to a folder or Node. See [Implied view access](/en/identity/scopes-reference/#implied-view-access).
...
1. Create a **MyProject** folder in each area the team uses, for example Routes, Docker containers, and Databases. Each resource type has its own folder tree, so a Routes folder does not cover containers. See [Folder support by resource type](/en/identity/scopes-reference/#folder-support-by-resource-type).
...
- Every resource in the folder and its subfolders, including resources created in or moved into it later. Gateway resolves folder grants on every request, and open pages pick up newly visible resources without signing in again.
...
- Containers and Deployments created from a Git source go into the chosen folder, also when the first build creates them later. While one waits for its first build it is listed in that folder and can be moved with **Move to folder...** or by dragging; the first build then creates it in the folder it is in at that moment. See [Folder placement of built workloads](/en/docker/git-builds/#folder-placement-of-built-workloads).
...
## Restrict Git access to repositories
...
To let a team build from one project, grant `integrations:gitlab:use` on that project next to the workload's own permissions: `use` on the project is enough to configure a Docker or Pages build source. Keep that grant in place: manual builds check it again, and automatic builds of a source saved in this release pause when the account that saved it loses `use`. API tokens and OAuth grants carry the same restrictions and never reach a repository that their owner cannot reach at that moment. See [Git integration restrictions](/en/identity/scopes-reference/#git-integration-restrictions) for the qualifier format and [Restrict repository access](/en/integrations/source-control/#restrict-repository-access) for what each operation checks.

## Operate workloads without adopting them

Resource-scoped Docker permissions work on containers that Gateway discovered but did not create. You can let a team restart a service and read its logs without handing the whole host, or the application's configuration, over to Gateway.
...
| Start, stop, or restart a Compose project | `docker:compose:manage:<node-id>/<project-id>` after the project is [adopted](/en/docker/compose/#adopt-an-external-project) | External projects are read-only until adoption; lifecycle actions apply to the whole project, not to individual Compose containers |
...
A standalone container is any container without Docker Compose project labels, including one started with `docker run`. Gateway gives it a stable resource identity that survives recreate and update operations performed through Gateway. If the container is removed and started again outside Gateway, Gateway records a new resource and revokes grants on the old identity. See [Containers that Gateway did not create](/en/docker/containers/#containers-that-gateway-did-not-create).

An external Compose project keeps its identity while Gateway observes it. If the project disappears from the Node, for example after `docker compose down`, Gateway removes it from inventory unless the project is placed in a Docker folder; when it reappears it receives a new identity, and grants on the old project identity no longer match. Place external projects that carry project-scoped grants in a Docker folder, or grant access through the folder.
