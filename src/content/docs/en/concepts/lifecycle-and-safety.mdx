...
License state follows the same principle. An expired key or an unreachable license service blocks new paid-only management actions after the documented grace, but Gateway does not stop, delete, or reconcile away workloads, Routes, managed databases, or published Pages deployments because of it. See [Downgrade and expiry](/en/reference/plans-and-entitlements/#downgrade-and-expiry).
