...
For external accounts, use the [Integrations overview](/en/integrations/overview/) to choose hosting, Git, SSH, or Cloudflare. Configure only the connections needed for your first workflow; a provider account is not required for manual node enrollment.
...
Gateway does not support SCIM yet. SCIM provisioning is planned, but no release date is guaranteed. Until it is available, manage users and groups in Gateway or automate the supported administration flow through the REST API with a scoped OAuth client if your identity system can call external APIs. OIDC sign-in proves identity; it does not provision or remove Gateway access by itself.
