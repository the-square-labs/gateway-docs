...
- **Create a VM through Gateway:** if you want to order a VM directly from the interface, connect a [hosting provider](/en/integrations/hosting-providers/) and choose hosted creation in **Add Node**. Gateway creates the VM and installs the daemon. Wait for node readiness, not just provider power-on.
...
| **Storage** | Gateway-managed PostgreSQL, Redis, and ClickHouse, managed object storage, and database backup jobs | Storage, backup, and stateful workload isolation |
...
Do not select a role merely because its installer happens to work on the host. A production role should match the host's intended trust boundary, network placement, storage, and recovery owner. In particular, Build Workers and Storage Nodes should normally be dedicated to their purpose.
...
- Storage Nodes need eligible durable storage and the required loop/mount capabilities. Do not assume an ordinary restricted LXC guest is sufficient.
- Build Workers need a dedicated host or outer unprivileged container and an approved egress profile.
- Relay Nodes need an address and port reachable from participating Gateway, Docker, Ingress, and Storage hosts. Gateway does not open the firewall or traverse NAT for you.
...
- **Storage:** confirm the selected storage root, capacity enforcement, and preflight completion before creating a managed database or managed storage.
