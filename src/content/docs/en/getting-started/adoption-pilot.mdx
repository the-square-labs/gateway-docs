...
Do not begin by moving the most critical system. A useful Gateway evaluation starts with one representative, reversible workload and proves the operating model before the scope grows. For short answers to common pre-purchase questions, see [Evaluating Gateway](/en/getting-started/evaluating-gateway/).
...
If the service already runs as a Docker Compose application, rehearse [adopting the Compose project](/en/docker/compose/#adopt-an-external-project) on a staging copy first. Rewrite unsupported keys such as `env_file` and host bind mounts, confirm that named volumes resolve to the same names, and record how one-off migration steps will run.
