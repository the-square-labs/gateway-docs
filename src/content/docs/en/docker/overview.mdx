...
[Workload Availability (HA)](/en/docker/availability/) is available on Business and Enterprise for eligible mount-free Containers, Deployments, and whole Compose Projects. It adds fixed-count replication or single-serving failover across independent Docker nodes to the existing resource identity.
...
Containers that Gateway did not create are still visible and operable. A standalone container started with `docker run` can be restarted and inspected under resource-scoped permissions without adoption, while an external Compose project stays read-only until it is adopted. See [Containers that Gateway did not create](/en/docker/containers/#containers-that-gateway-did-not-create) and [Adopt an external project](/en/docker/compose/#adopt-an-external-project).
...
Workloads reach databases in one of two ways. A [managed database](/en/databases/managed-databases/) can be bound to a Container, Deployment, or Compose service, and Gateway injects connection values for a dedicated engine identity. An external database is reached through connection values that you store as workload secrets. See [Managed and external databases at a glance](/en/databases/overview/#managed-and-external-databases-at-a-glance).
