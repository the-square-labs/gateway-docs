...
On a Docker Node whose default log driver is `json-file`, Gateway limits the logs of each container it creates or recreates without its own logging configuration to three files of 50 MB. Other log drivers are left unchanged, and Compose Projects keep Docker's defaults.
