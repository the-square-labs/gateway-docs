...
- `logging` can set only `driver` and `options`; the driver must be `json-file`, `local`, or `none`, and options are limited to `max-size`, `max-file`, and `compress` (`none` takes no options);
...
On a node whose default log driver is `json-file`, the Docker daemon gives every service without `logging` json-file rotation at 50 MB × 3, the same limit single containers get. To keep a service's logs longer, set your own `max-size` and `max-file`; `driver: json-file` without options keeps Docker's unlimited default. Docker cannot change the log settings of an existing container, so the first apply after the Docker daemon update recreates each service once. Applying a new revision already recreates every service, so this matters only when you re-apply an unchanged revision. A node whose Docker daemon has not been updated yet applies revisions without their `logging` sections and keeps Docker's defaults.
