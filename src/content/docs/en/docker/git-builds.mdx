...
Configure the connector using [Git hosting](/en/integrations/source-control/). That guide also separates repository permissions from the target permissions needed to start saved builds and read their history. Registry credentials are covered under [Container registries](/en/docker/registries/).
...
While a build is rolled out to a Container, Deployment, or Compose Project, Gateway locks that target: lifecycle and configuration changes, secret changes, Availability changes, and migrations are refused with `BUILD_ROLLOUT_IN_PROGRESS` until the rollout succeeds, fails, or is cancelled, and the Console shows a banner on the target. The lock survives a Gateway restart. If the target is busy with another operation when the rollout starts, the rollout waits up to 5 minutes and then fails with `BUILD_ROLLOUT_TARGET_BUSY`. Pages releases are not locked this way. During a Gateway update, builds triggered by webhooks or polling are deferred and start once after the update.
...
If a build cannot start, check source authorization, worker availability, entitlement, and integration state. A cancelled build that reads "Build paused: <user> no longer has use on <repo>" means the account that saved the source lost `integrations:<provider>:use` on the repository; see [Who can build and view results?](/en/integrations/source-control/#who-can-build-and-view-results). If it fails after start, use the phase and logs to separate dependency, secret, policy, build-definition, or artifact-publication failures. Correct the source or build configuration, then run a new build. Retrying produces a new operation and preserves the previous history.
