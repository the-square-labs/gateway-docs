...
| Gateway | License service `license.thesqlabs.com` over HTTPS | Registration and heartbeats; every Gateway update on every plan; paid key activation |
| Storage client | managed storage S3 port (9000 by default), SFTP (8022), FTP (2121) and its passive range | Only when publication or a file protocol is deliberately enabled; these listeners bind on every member |
| Distributed storage member | other members' service IP on the S3 port | Always for a distributed managed storage cluster |
