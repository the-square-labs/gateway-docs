...
- **Login succeeds but Gateway returns 403:** the account needs `mcp:use` and at least one effective resource scope. A grant limited to a folder sees only that folder's resources; creating elsewhere is denied.
