...
Cancellation stops the active build operation but does not delete previous Deployments or move an existing production Tag. A retry resolves the branch again according to the source workflow, so verify the exact commit attached to the resulting build instead of assuming it used the originally observed branch head.
