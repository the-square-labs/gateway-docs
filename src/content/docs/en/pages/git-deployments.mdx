...
Connect and restrict the repository first using [Git hosting](/en/integrations/source-control/). Connector access, Pages deployment permissions, and permission to view build history are separate checks.
...
Enable automatic builds when every relevant source update should produce a candidate. Enable automatic deployment only when every successful candidate may safely move the configured Tag. For documentation and marketing sites, automatic build with explicit production promotion allows the immutable preview to be reviewed first. Automatic builds of a source saved since 2.11.0-rc.11 run only while the account that saved it keeps `integrations:<provider>:use` on the repository; otherwise the build history shows "Build paused: <user> no longer has use on <repo>". See [Who can build and view results?](/en/integrations/source-control/#who-can-build-and-view-results).
...
During a Gateway update, builds triggered by webhooks or polling are deferred and start once after the update completes.
