...
| Daemon client certificates on every Node | 365 days | Checked daily by the daemon and renewed 7 days before expiry; the new certificate takes effect when the daemon reconnects with it |

A Node must stay connected to renew its certificate. Keep Nodes online, and investigate a Node that has been offline for a long time before its certificate expires.
