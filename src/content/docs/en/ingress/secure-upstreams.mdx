...
An additional binding can also target ready [managed object storage](/en/storage/overview/) (**Managed S3 storage**): private S3 through the Relay, without a shared Docker network or a published S3 port. Clients still need S3 credentials. Reference it in Advanced configuration as `{{additionalSecureLinks.<name>}}`; a binding that Advanced configuration still references cannot be deleted. Creating one requires Route edit access and view access to the storage, and managed storage cannot be a Route's main upstream.
