...
4. **Route.** `create_route` or `update_route` with a stable target and the SSL certificate ID (never a PKI certificate ID). Omit `nodeId` for a registered Domain: the Route uses the Domain's Ingress Node. Without one, `list_route_ingress_nodes` lists the Nodes you may create Routes on; no Node permission is needed. Target shapes, Additional Routes, Secure Links, and templates: [Routes and upstreams](references/routes-and-upstreams.md).
